...
for i_sub = 1:nsubs
    for i_cond = 1:nconds
        erp_out(:,1,:,i_cond,i_sub) = mean(     ALLEEG( 1 + 3*((i_sub-1)*nconds + (i_cond-1)) ).data         ,3)'; %targets
        erp_out(:,2,:,i_cond,i_sub) = mean(     ALLEEG( 2 + 3*((i_sub-1)*nconds + (i_cond-1)) ).data         ,3)'; %standrads
        erp_out(:,3,:,i_cond,i_sub) = mean(     ALLEEG( 3 + 3*((i_sub-1)*nconds + (i_cond-1)) ).data         ,3)'; %standrads
...
p3 =  fill([400 600 600 400],[-7 -7 15 15],'y') ; 
set(p3,'edgecolor','none');
 uistack(p3,'bottom'); 
    ...
    %add condition back in later!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     Diffmeans = squeeze(mean(erp_diff_out(:,electrode,:),3));
        Diffstds = squeeze(std(erp_diff_out(:,electrode,:),[],3) )./sqrt(nsubs);
     Diffmeans2 = squeeze(mean(erp_diff_out2(:,electrode,:),3));
        Diffstds2 = squeeze(std(erp_diff_out2(:,electrode,:),[],3) )./sqrt(nsubs);
...
% subplot(2,1,1);
%         electrode = 9;
%         boundedline(EEG.times,squeeze(mean(erp_diff_out(:,electrode,1,:),4)), squeeze(std(erp_diff_out(:,electrode,1,:),[],4))./sqrt(nsubs),'g' ,...
%                     EEG.times,squeeze(mean(erp_diff_out(:,electrode,2,:),4)), squeeze(std(erp_diff_out(:,electrode,2,:),[],4))./sqrt(nsubs),'b');% ,...
%                     % EEG.times,squeeze(mean(erp_diff_out(:,electrode,3,:),4)), squeeze(std(erp_diff_out(:,electrode,3,:),[],4))./sqrt(nsubs),'r');
%                set(gca,'Color',[1 1 1]);
%         set(gca,'YDir','reverse'); 
% % line('XData', [100 100], 'YData', [-6 9], 'LineStyle', '-','LineWidth', 2, 'Color','r');
% % line('XData', [150 150], 'YData', [-6 9], 'LineStyle', '-','LineWidth', 2, 'Color','r');
% line('XData', [325 325], 'YData', [-6 9], 'LineStyle', '-','LineWidth', 2, 'Color','m');
% line('XData', [400 400], 'YData', [-6 9], 'LineStyle', '-','LineWidth', 2, 'Color','m');
%        legend('Video','Static','Location','SouthEast'); 
%         text( -6, 12, 'P2(175-275) labelled')
%         
%         axis tight; ylim([-6 9]);
%         line([-200 1000],[0 0],'color','k');
%         line([0 0],[-6 9],'color','k');
...
%                set(gca,'Color',[1 1 1]);
%         set(gca,'YDir','reverse'); 
% % line('XData', [100 100], 'YData', [-5 10], 'LineStyle', '-','LineWidth', 2, 'Color','r');
% % line('XData', [150 150], 'YData', [-5 10], 'LineStyle', '-','LineWidth', 2, 'Color','r');
% line('XData', [400 400], 'YData', [-5 10], 'LineStyle', '-','LineWidth', 2, 'Color','m');
% line('XData', [600 600], 'YData', [-5 10], 'LineStyle', '-','LineWidth', 2, 'Color','m');
%         legend('Video','Static','Location','SouthEast'); 
%        
%         axis tight; ylim([-5 20]);
%         line([-200 1000],[0 0],'color','k');
%         line([0 0],[-5 10],'color','k');
